...
export default function UpdateProfile({
  ...
  const handleUpdate = async (_e: any) => {
    ...
    if (
      SuggestData.some(
        (user: any) => user.id !== userid && user.username === userName
      )
    ) {
    ...
    <>
      {isInputClicked && (
        <div className=" absolute -top-[46px] left-0 right-0 bg-black h-[36rem] pt-6 z-50">
          <div
            className={` right-0 mx-auto  px-4 pb-5 pt-2  shadow md970:w-[100%] sm650:-top-9 rounded-t-2xl h-[37rem]  overflow-y-auto z-[60] ${
