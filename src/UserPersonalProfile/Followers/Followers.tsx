...
export default function Followers({ userData, SuggestData }: Props) {
  ...
  const suggestedUsers =
    SuggestData && SuggestData.filter((user: any) => followerid.has(user.id));
  ...
  const [hoverStates, setHoverStates] = useState(
    Array(suggestedUsers.length).fill(false)
  );
  ...
  const handleRemove = async (dataid: string) => {
    try {
      const userDocRefFollowing = doc(db, "users", userid as string);
      await updateDoc(userDocRefFollowing, {
        Followers: arrayRemove(dataid), // Remove the user ID from Following array
      });

      const userDocRefFollowers = doc(db, "users", dataid);
      await updateDoc(userDocRefFollowers, {
        Following: arrayRemove(userid), // Remove the current user ID from the target user's Followers array
      });
        ...
        {isLoading ? (
          ...
          suggestedUsers.map((user: any) => (
            <Link
              ...
              key={user.id}
              className={`select-none py-1 transition-all ${
                theme ? "hover:bg-[#ffffff16]" : "hover:bg-[#00000010]"
              }`}
            >
                  ...
                  <aside className="flex gap-2">
                    {userData.Following?.includes(user.id) ? (
                      <button
                        ...
                        className={`font-medium py-[6px] px-3 rounded-full ${
                          hoverStates[user.id]
                            ? "bg-red-500 text-white"
                            : theme
                            ? "bg-[#ffffff] text-black"
                            : "bg-[#000000a5] text-white"
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`font-medium py-[6px] px-4 rounded-full ${
                          theme
                            ? "bg-[#ffffff] text-black"
                            : "bg-[#000000a5] text-white"
                        }`}
                    ...
                    <Popover className="relative">
                      ...
                      <Popover.Panel
                        className={`absolute  right-7 w-52 top-0 pt-2 pb-4  rounded-2xl ${
                          theme
                      ...
                      >
                        <Popover.Button
                          onClick={() =>
                            handleShare(user.username, user.fullName)
                          }
                          className={`flex gap-2 py-1 mt-2 w-full px-3 ${
                            theme
                              ? "hover:bg-[#ffffff16]"
                              : "hover:bg-[#00000010]"
                          }`}
                        >
                          <span className=" text-xl mt-[3px]">
                        ...
                        >
                          ...
                          <p className=" font-semibold  text-left whitespace-nowrap text-ellipsis overflow-hidden w-full ">
