    ...
    useEffect(() => {
        const handleGetPost = (snapshot: any) => {
            const data = snapshot.docs
                ...
                .filter((repost: any) => repost.RepostAuthor === userid as string); // Filter rePosts by userid
            setIsLoading(false);
            setRePostData(data);
        };
        const unsubscribePost = onSnapshot(collection(db, "Repost",), handleGetPost);
        return () => {
            unsubscribePost()
        };
    }, []);
    console.log(isLoading, repostData);
