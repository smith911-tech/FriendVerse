...
export default function PostController({
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (
      let i = 0;
      i < files.length && uploadedImages.length + newImages.length < 4;
      i++
    ) {
    ...
    try {
      ...
      if (uploadedImages && uploadedImages.length > 0) {
        ...
        for (const imageFile of selectedImgFiles) {
          const storageRef = ref(
            storage,
            `Posts/${postRef.id}/${imageFile.name}`
          );
      ...
      if (UploadedVideo) {
        const videoStorageRef = ref(
          storage,
          `Posts/${postRef.id}/${selectedVidFile?.name}`
        );
        const videoUploadTask = uploadBytesResumable(
          videoStorageRef,
          selectedVidFile as any
        );
  ...
  return (
    <>
      {isInputClicked && (
        <div
          className={`absolute top-0 left-0 right-0 mx-auto p-4 z-[30] rounded-2xl shadow md970:w-[90%] sm650:-top-9  ${
        ...
        >
          <section className="flex justify-between mb-1 select-none">
            ...
            <span
              className="text-xl bg-[#f0f2f5] mb-2 rounded-full text-[#0000009b] px-1 py-1 cursor-pointer smm500:text-lg"
          ...
          <main className="overflow-y-auto h-[280px] ">
            ...
            <section className=" pr-2">
              <textarea
                ref={inputRef}
                className={`w-full text-xl pt-2 mt-2 outline-none smm500:text-lg ${
                  theme ? "bg-black text-white" : "bg-white text-black"
                }`}
                name=""
                id=""
              ...
              />
              <div
                className={`text-right select-none text-[#7e7e7e] smm500:text-sm mt-1 ${
                  characterCount === 0 ? "text-[#ff0000a8]" : "text-[#7e7e7e]"
                }`}
              >
                {characterCount}/{820}
              </div>
              ...
              <section className="flex flex-col-reverse">
                {uploadedImages.map((imageUrl, index) => (
                  <div key={index} className="relative">
                    <LazyLoadImage
                      effect="blur"
                      className="w-full mb-2 object-cover h-full"
                      key={index}
                      src={imageUrl}
                      alt={`Uploaded ${index}`}
                    />
                    <span
                      className=" absolute text-xl p-1 rounded-full top-2 right-3 text-[#fff] bg-[#000000c1] cursor-pointer"
              ...
              {showCodeBlock && (
                <div className="mt-4">
                  <textarea
                    ...
                    className={`w-full p-2 border-2 border-solid ${
                      theme
                        ? "border-[#ffffffa7] bg-black"
          ...
          <section
            className={` text-2xl flex justify-between border border-solid gap-2 py-2 px-3 mb-3 smm500:border-[0.1px] smm500:py-1 smm500:px-2
                    ${theme ? "border-[#ffffff84]" : "border-[#000000b6] "}`}
          >
            <h2
              className={`font-medium text-xl smm500:text-base select-none 
                        ${theme ? "text-white" : " text-[#000000b8] "}`}
            >
              Add to your post
            </h2>
            <div className="flex gap-3 ">
              <label
                htmlFor="UploadImg"
                className=" cursor-pointer text-[#45bd62] mt-1 smm500:text-lg"
              >
                <abbr title="Photo">
                  <TfiGallery />
                </abbr>
              ...
              <label
                htmlFor="UploadVideo"
                className=" cursor-pointer text-[#f5533d] mt-1 smm500:text-lg"
              >
                <abbr title="Video">
                  <ImFileVideo />
                </abbr>
          ...
          <button
            ...
            className={`my-3 text-center w-full py-[6px] bg-[#3b82f6] text-white text-xl font-medium  smm500:py-1 smm500:text-lg select-none  
                    ${disableBtn ? " brightness-75 cursor-not-allowed" : ""}`}
          >
            Post
          </button>
          {uploadProgress > 0 && (
          ...
          {Loading && (
            <div
              className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 progressPost inset-4 w-full h-full rounded-2xl
