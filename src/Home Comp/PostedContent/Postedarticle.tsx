  ...
  const renderArticleContent = (content: string) => {
    ...
    return parts.map((textOrUrl, index) => {
      if (textOrUrl.match(urlRegex)) {
        ...
        // It's regular text
        return <span key={index}>{textOrUrl}</span>;
      }
    });
  };
        ...
        className={`pt-2  px-2 overflow-auto
                ${
                  window.location.pathname !== `/Post/${post.id}`
                    ? theme
                      ? "hover:bg-[#ffffff0f] cursor-pointer"
                      : "hover:bg-[#00000017] cursor-pointer"
                    : ""
                }`}
          ...
          {showFullArticle ? (
            <div>
              {paragraphs.map((paragraph: string, index: number) => (
                <p
                  key={index}
                  className={`text-[15px] font-normal font-sans break-words ${
