...
export default function Postsection({ SuggestData }: Props) {
  ...
  const CombinedData = [
    ...Posts,
    ...repostedData.filter((repost) =>
      Posts.some((post) => post.id === repost.id)
    ),
  ];
      ...
      <section className="md970:w-[90%] block mb-0 mx-auto mt-4">
        {isLoading ? ( // Render loading state
          ...
          CombinedData.map((post) => {
            const authorData = SuggestData.find(
              (user: any) => user.id === post.author
            );
            const Repostauthor =
              SuggestData &&
              SuggestData?.find((user: any) => user?.id === post?.RepostAuthor);
            if (authorData) {
                  ...
                  >
                    ...
                    >
                      {Repostauthor && (
                        <Link
                          to={`${
                            userid !== post.RepostAuthor
                              ? `/User/${Repostauthor.username}`
                              : "/Profile"
                          }`}
                          className="flex ml-4 text-sm gap-1 pb-4"
                        >
                        ...
                        >
                          <div className=" flex flex-col w-full">
                            ...
                            <Popover.Button
                              ...
                              className="flex gap-1 w-full py-2 pl-3 hover:bg-[#00000076] cursor-pointer"
                            >
                              <FiShare2 className="text-2xl px-1" />{" "}
                              <p>Share</p>
                            </Popover.Button>
                            {userid === post.author && (
                              <Popover.Button
