...
export default function RequestContent({ userData, SuggestData }: userData) {
  ...
  return (
    <main
      className={`${
        theme ? "bg-black text-[#ffffffe0]" : "bg-[#ffff] text-black"
      } mb-10`}
    >
      <article className=" py-2 px-1">
        {SuggestData.length > 0 ? (
          SuggestData.filter((data: any) => data.id !== userid).map(
            (data: any) => (
              <Link to={`/User/${data.username}`} key={data.id}>
                ...
                <div
                  className=" float-right -mt-12 mr-2"
                ...
                <p className="font-medium ml-[52px] text-sm -mt-1 mb-6">
                  {data.bio}
                </p>
              </Link>
            )
