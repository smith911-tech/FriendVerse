  ...
  return (
    <header>
      <section className="flex gap-2 pb-4">
        <Popover.Button
          className={` text-4xl pb-2 rounded-full h-9
        ...
        <input
          type="text"
          className={`w-full h-9 outline-none rounded-lg px-3 
      ...
      {searchTerm && (
        <div
          className={`absolute w-full   shadow z-10 -ml-2 overflow-y-auto overflow-x-hidden ${
            getSuggestions(searchTerm).length === 0 ? " h-[inherit] " : "h-44 "
          } 
              ...
              .map((data: any) => (
                <Link to={`/User/${data.username}`}>
                  ...
                  >
                    ...
                    <div className="full">
                      <p className="text-left font-semibold whitespace-nowrap overflow-hidden w-[100%] text-ellipsis flex">
                        {data.fullName}
                        {data && data.Verify && (
